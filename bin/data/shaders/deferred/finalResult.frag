...
vec4 calculatePointLight_BlinnPhong(uint materialParams, vec3 position, vec3 normal, vec3 lightColor, vec3 lightPosition, float size, float power, vec3 worldPosition, bool isSun)
{
...
{		
	...
	vec4 ws = getWorldPosition();
	vec3 worldPosition = vec3(ws/ws.w);
	vec4 worldPositionScreenSpace = viewMatrix * ws;
	worldPositionScreenSpace.xyz /= worldPositionScreenSpace.w;
	...
	if((parameters & IS_SHADELESS) != IS_SHADELESS)
	{
		...
		vec3 vecPosition = vec3(worldPositionScreenSpace);
		...
		vec3 lightPosition = vec3(0.8,2,0.8);

		lighting += calculatePointLight_BlinnPhong(parameters, vecPosition, vecNormal, lightColor, lightPosition, size, power, worldPosition, false);
		...
		lightPosition = vec3(200, 500, 200);

		lighting += calculatePointLight_BlinnPhong(parameters, vecPosition, vecNormal, lightColor, lightPosition, size, power, worldPosition, true);
